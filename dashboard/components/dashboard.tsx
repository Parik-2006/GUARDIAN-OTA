...
function DeviceModal({ d, onClose }: { d: DeviceState; onClose: () => void }) {
  ...
  const handleOTA = async () => {
    ...
    await deployFirmware({
      version: "1.0.0",
      firmwareUrl: "https://your-bucket.s3.amazonaws.com/guardian-ota.bin",
      firmwareHash: "sha256:abc123",
      signatureB64: "manual-override-sig",
      canaryPercent: 10
    }).catch(console.error);
...
function DashboardView({ fleet }: { fleet:DeviceState[] }) {
  ...
  const kpis = [
    {label:"Update Success Rate",val:"98.2%",  sub:"+1.4% ╬ö",      icon:"check_circle", subCol:P.sage,    col:P.cognac,  hi:false},
              ...
              <div key={g.label} style={{ position:"absolute",left:`${g.x}%`,top:`${g.y}%`,transform:"translate(-50%,-50%)" }}>
                ...
                <div style={{ position:"absolute",bottom:13,left:"50%",transform:"translateX(-50%)",background:"rgba(22,18,16,0.92)",border:`1px solid ${P.bMid}`,borderRadius:3,padding:"6px 10px",width:128,whiteSpace:"nowrap" }}>
                  ...
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"end" }}>
                    ...
                    <span style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:"0.5rem",color:P.sage }}>ΓùÅ ONLINE</span>
                  </div>
                </div>
              </div>
            ))}
...
function TerminalView() {
  ...
  const [lines,setLines]=useState<Line[]>([
    {type:"info",text:"GUARDIAN-OTA Command Interface v4.1.0 ΓÇö Initialized"},
    ...
    <div style={{ display:"grid",gridTemplateColumns:"1fr 325px",gap:9,padding:15,flex:1,overflow:"hidden" }}>
      ...
      >
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:15,flex:1 }}>
          {[["TARGET PAYLOAD",["v2.4.1-stable (ESP32)","v1.8.5-patch (nRF52840)"]],["TARGET FLEET / CHIPSET",["Production ΓÇö ESP32-WROOM","Staging ΓÇö ESP32-WROOM","Alpha Testers ΓÇö All"]]].map(([label,opts]:any)=>(
            <div key={label}>
...
function VerificationView() {
  ...
  const cryptoLog=[
    ...
    {ts:"14:02:44.201",node:"NODE [ESP32-D005]",text:"FATAL_ERROR ΓÇö Hash mismatch detected.",          col:P.burg},
